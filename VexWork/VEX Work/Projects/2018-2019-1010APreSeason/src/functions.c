...
void cap(int value, int cap){
	if(value > cap){value = cap;} //if the value is greater than the cap, the value becomes the cap
	if(value < -(cap)){value = -(cap);} //if the value is less than the negaative of the cap, it becomes the negative cap.
}
...
void AutoDrive(bool direction, int targetValue, int timeout){
  ...
  while(EncAvg < targetValue || (millis()-startTime)<timeout) { //while encoder average is less than target, or timeout
    ...
    moveDrive((LPower * mult), (RPower * mult), 127);//send to the drive the power times the mult with a 127 cap
